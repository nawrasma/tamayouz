...
class ManagerController extends Controller
{
    ...
    public function dashboradPage()
    {
        ...
        $projects= Project::where('season_id',$seasons[0]->id )->orderBy('created_at','DESC')->get();   //->sortByDesc('created_at')
        ...
        $ResArray=[ 'categoiesNum'=>count($Categories),'projectsNum'=>count($projects), 
        ...
        return view('master.dashboardManager',$ResArray);
    }
    ...
    public function all_projects()
    {
        ...
        $userRole=Auth::user()->getRoleUser();
        $seasons= Season::where('user_id',Auth::user()->id)->get();
        $projects = Project::where('season_id',$seasons[0]->id )->get();
    ...
    public function single_project($proID)
    {
        ...
        $seasons= Season::where('user_id',Auth::user()->id)->get();
        $categories= Category::all();
        $project = Project::where([['id','=' ,$proID],['season_id','=', $seasons[0]->id ]])->first();
    ...
    public function destroyProject()
    {
        ...
        return redirect()->action('ManagerController@all_projects');
    }
    ...
    public function search_project()
    {
        ...
        {
          $query->orWhere([ [$column, 'LIKE', '%' . request('inp_search') . '%'],['season_id','=', $seasons[0]->id]  ]);
        }
        $result = $query->get();
       // dd($result);        
